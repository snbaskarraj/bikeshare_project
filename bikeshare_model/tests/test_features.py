...
def test_weathersit_imputer():
    ...
    assert (
        transformed_df["weathersit"].mode()[0] == transformed_df.loc[1, "weathersit"]
...
def test_weekday_onehot_encoder():
    ...
    assert all(
        col.startswith("weekday_") for col in transformed_df.columns
    )  # Check if new columns have the correct prefix
